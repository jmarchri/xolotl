...
unsigned int HDF5File::toHDF5AccessMode(AccessMode mode) {
...
herr_t HDF5File::BuildCurrStackLevelErrorString(uint32_t n,
		const H5E_error2_t* eptr, void* client_data) {
...
std::string HDF5File::BuildHDF5ErrorString(void) {

	std::ostringstream estr;

	auto ok = H5Ewalk(H5E_DEFAULT, H5E_WALK_UPWARD,
			BuildCurrStackLevelErrorString, &estr);
	if (ok < 0) {
...
void HDF5File::Open(fs::path _path, AccessMode _mode, MPI_Comm _comm,
		bool par) {
	...
	if ((_mode == AccessMode::OpenReadOnly)
			or (_mode == AccessMode::OpenReadWrite)) {
...
bool HDF5File::hasGroup(fs::path path) const {
